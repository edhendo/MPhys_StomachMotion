...
THIS IS THE MOST RECENT t-SNE SCRIPT
        ...
        for y in range(thiccShell.shape[1]):
            for z in range(thiccShell.shape[2]):
                if (thiccShell[x,y,z] == 1):
                    pixels += 1;
    extracted_DVF_Data = np.ndarray((pixels,6*10));     # This final number needs to be x*10, x = num of features (x,y,z,phi,theta,r), 10 = num of DVFs
    pixelNum = 0;
    for x in range(thiccShell.shape[0]):
        for y in range(thiccShell.shape[1]):
            for z in range(thiccShell.shape[2]):
...
'''
clusters = 5
kmeans5 = KMeans(n_clusters=clusters, random_state=1000000).fit(tsneResult);
...
for i in range(verts.shape[0]):
    if kmeans5.labels_[i] == 0:
        tsne_vertex_colours5[i] = [60/255,68/255,216/255]; # blue
    if kmeans5.labels_[i] == 1:
        tsne_vertex_colours5[i] = [16/255,140/255,31/255]; # green
    if kmeans5.labels_[i] == 2:
        tsne_vertex_colours5[i] = [173/255,19/255,19/255]; # red
    if kmeans5.labels_[i] == 3:
        tsne_vertex_colours5[i] = [255/255,246/255,7/255]; # yellow
    if kmeans5.labels_[i] == 4:
        tsne_vertex_colours5[i] = [139/255,19/255,191/255];# purple
    
    ''' # this bit is for colour map assignment, not set rgb values as above
    for rgb in range(3):
